...
system.beforeEvents.startup.subscribe((data) => {
    ...
    command.registerEnum("xs:interactType", ["minecraft:place", "minecraft:break", "minecraft:interact_block", "minecraft:interact_entity", "minecraft:attack_entity", "minecraft:pickup", "minecraft:item_use", "minecraft:item_drop", "minecraft:chest_use", "minecraft:healing", "minecraft:chat", "minecraft:taming", "minecraft:explosion"])
    ...
    command.registerEnum("xs:datalist", ["bandata_list", "playersdata_list"])
    command.registerEnum("xs:datatype", ["export"])
    command.registerEnum("xs:queueBanOptions", ["add", "remove", "list"])
        ...
        command.registerCommand({
            ...
            if (target.length === 1) {
                if (param === "id" || param === "pfid" || param === "pos") {
                    ...
                    if (param === "pfid") {
                        origin.sourceEntity.sendMessage(`${target[0].name} : ${target.persistentId}`)
                        return {
                            status: CustomCommandStatus.Success,
    ...
    if (pack["command:dimensiontp"] === true) {
        command.registerCommand({
            name: "xs:dimensiontp",
    ...
    if (pack["command:transfers"] === true) {
        command.registerCommand({
            name: "xs:transfers",
    ...
    if (pack["command:ban"] === true) {
        command.registerCommand({
            name: "xs:q-ban",
            description: "プレイヤーのアクセスを禁止します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [{ name: "selector", type: CustomCommandParamType.PlayerSelector }],
            optionalParameters: []
        }, (origin, players) => {
            ...
            if (sender instanceof Player) {
                for (const player of players) {
                    if (player.playerPermissionLevel !== PlayerPermissionLevel.Operator) {
                        const p = playersList.getPlayer(player.persistentId)
                        p.ban()
                        system.run(() => { player.runCommand("kick @s `このサーバーへのアクセスが禁止されています`") })
                    }
                }
                return {
                    status: CustomCommandStatus.Success,
                    message: `${display(players, 10)}のアクセスを禁止しました`
                }
            }
        ...
        command.registerCommand({
            ...
            description: "プレイヤーのアクセスを禁止します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [{ name: "name | list", type: CustomCommandParamType.String }],
            optionalParameters: []
        }, (origin, target) => {
            const sender = origin.sourceEntity
            if (sender instanceof Player) {
                if (target === "list") {
                    const ids = playersList.getAllBanPlayers()
                    let list = []
                    if (ids.length) {
                        ids.forEach(id => {
                            list.push(`- ${id._name} : ${id._persistentId}`)
                        })
                        return {
                            status: CustomCommandStatus.Success,
                    ...
                    const id = playersList.getPlayerFromName(id)
                    if (id !== undefined) {
                        if (!id.hasBanned()) {
                            id.ban()
                            if (world.getAllPlayers().find(p => p.name === id.getName())) {
                                const p = world.getAllPlayers().find(p => p.name === id.getName())
                                system.run(() => { p.runCommand("kick @s `このサーバーへのアクセスが禁止されています`") })
                            }
                            return {
                                status: CustomCommandStatus.Success,
                                message: `${id.getName()} のアクセスを禁止しました`
                            }
                        }
                        else return {
                            status: CustomCommandStatus.Failure,
        ...
        command.registerCommand({
            ...
            if (sender instanceof Player) {
                if (isNaN(target)) {
                    const id = playersList.getPlayerFromName(target)
                    if (id !== undefined) {
                        if (!id.hasBanned()) {
                            return {
                                status: CustomCommandStatus.Failure,
    ...
    if (pack["command:extinguishfire"] === true) {
        command.registerCommand({
        ...
        command.registerCommand({
            ...
            if (mode === "arrow" || mode === "box" || mode === "circle" || mode === "line" || mode === "sphere" || mode === "text") {
                ...
                if (mode === "arrow") {
                    ...
                    if (color) draw.color = { red: rgb.x - 0.5, green: rgb.y, blue: rgb.z - 0.5, alpha: 1 };
    ...
    if (pack["command:cleardraw"] === true) {
        command.registerCommand({
            name: "xs:cleardraw",
    ...
    if (pack["command:client-particle"] === true) {
        command.registerCommand({
    ...
    if (pack["command:maxhealth"] === true) {
        command.registerCommand({
            name: "xs:maxhealth",
    ...
    if (pack["command:summonitem"] === true) {
        command.registerCommand({
            name: "xs:summonitem",
    ...
    if (pack["command:interact"] === true) {
        command.registerCommand({
            ...
            const entity = origin.sourceEntity
            if (targets.length) {
                if (type === "minecraft:place" ||
                    type === "minecraft:break" ||
                    type === "minecraft:interact_block" ||
                    type === "minecraft:interact_entity" ||
                    type === "minecraft:attack_entity" ||
                    type === "minecraft:pickup" ||
                    type === "minecraft:item_use" ||
                    type === "minecraft:item_drop" ||
                    type === "minecraft:chest_use" ||
                    type === "minecraft:healing" ||
                    type === "minecraft:chat" ||
                    type === "minecraft:taming" ||
    ...
    if (pack["command:ping"] === true) {
        command.registerCommand({
            name: "xs:ping",
            description: "現在のpingを計測します",
            permissionLevel: CommandPermissionLevel.Any,
            mandatoryParameters: []
        }, (origin, datas) => {
            const entity = origin.sourceEntity
            if (entity instanceof Player) {
                const ping = entity.getPing()
                entity.sendMessage(`現在のping: ${ping} ms`)
            }
            return {
                status: CustomCommandStatus.Success,
            };
    ...
    if (pack["command:seed"] === true) {
        command.registerCommand({
            name: "xs:seed",
            description: "シード値を表示します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: []
        }, (origin, datas) => {
            const entity = origin.sourceEntity
            if (entity instanceof Player) {
                entity.sendMessage(`シード値: ${world.seed}`)
            }
            return {
                status: CustomCommandStatus.Success
            };
    ...
    if (pack["command:worldproperty"] === true) {
        command.registerCommand({
            name: "xs:worldproperty",
            description: "重要なデータを外部にエクスポートします",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [{ name: "xs:datalist", type: CustomCommandParamType.Enum }, { name: "xs:datatype", type: CustomCommandParamType.Enum }]
        }, (origin, data, type) => {
            const entity = origin.sourceEntity
            if (entity instanceof Player) {
                if (data === "bandata_list" || data === "playersdata_list") {
                    if (type === "export") {
                        let dt;
                        if (data === "bandata_list") dt = playersList.getBanList()
                        if (data === "playersdata_list") dt = playersList.getPlayersList()
                        console.info(`エクスポート完了: (データ名: ${data}) | ${JSON.stringify(dt)}`)
                        return {
                            status: CustomCommandStatus.Success,
                            message: `指定されたデータをエクスポートしました (コンテンツログの履歴からコピーできます。表示されない場合は設定からコンテンツログの表示レベルを[情報]に設定してください。)`
                        }
                    } else return {
                        status: CustomCommandStatus.Failure,
                        message: `引数が存在しません`
    ...
    if (pack["command:textprimitive"] === true) {
        command.registerCommand({
            name: "xs:textprimitive-create",
            description: "フロートテキストを作成します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [{ name: "location", type: CustomCommandParamType.Location }, { name: "text|rawtext", type: CustomCommandParamType.String }],
            optionalParameters: [
                { name: "rotation", type: CustomCommandParamType.Location },
                { name: "scale", type: CustomCommandParamType.Float },
                { name: "visibleTo", type: CustomCommandParamType.PlayerSelector },
        ...
        }, (origin, location, texts, rotation, scale, visible, userot, back, textback) => {
            const or = origin.sourceBlock ?? origin.sourceEntity
            const dim = or.dimension
            if (origin.sourceType === CustomCommandSource.Block || origin.sourceType === CustomCommandSource.Entity) {
                if (scale === undefined || (-1000 <= scale && scale <= 1000)) {
                    let er = false;
                    try {
                        const text = new TextPrimitive({ dimension: dim, x: location.x, y: location.y + 0.5, z: location.z }, texts.startsWith("{") ? JSON.parse(texts) : texts)
                        text.rotation = rotation ?? { x: 0, y: 0, z: 0 }
                        text.scale = scale ?? 1
                        text.visibleTo = visible ?? []
                        text.useRotation = userot ?? false
                        text.backfaceVisible = back ?? false
                        text.textBackfaceVisible = textback ?? false
                    ...
                    if (er) {
                        return {
                            status: CustomCommandStatus.Failure,
                            message: `rawtext構文に誤りがあります`
                        }
                    }
                    else {
                        return {
                            status: CustomCommandStatus.Success,
                            message: `フロートテキスト を生成しました`
                        }
                    }
                }
                else return {
                    status: CustomCommandStatus.Failure,
                    message: `スケールは -1000 ~ 1000 の大きさです`
                }
            }
        })
        command.registerCommand({
            name: "xs:textprimitive-color",
            description: "フロートテキストの色を変更します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [
                { name: "location", type: CustomCommandParamType.Location },
                { name: "RGB", type: CustomCommandParamType.Location },
                { name: "alpha", type: CustomCommandParamType.Float },
            ],
            optionalParameters: [
                { name: "bg_RGB", type: CustomCommandParamType.Location },
                { name: "bg_alpha", type: CustomCommandParamType.Float },
                { name: "random", type: CustomCommandParamType.Boolean },
                { name: "bg_random", type: CustomCommandParamType.Boolean },
            ]
        }, (origin, location, RGB, a, bg_RGB, bga, random, bgrandom) => {
            let r = RGB.x
            let g = RGB.y
            let b = RGB.z
            let bgr = bg_RGB.x
            ...
            if (origin.sourceType === CustomCommandSource.Block || origin.sourceType === CustomCommandSource.Entity) {
                ...
                if (
                    ...
                    (g >= 0 && g <= 1) && (bgg >= 0 && bgg <= 1) &&
                    (b >= 0 && b <= 1) && (bgb >= 0 && bgb <= 1)) {
                    const texts = world.primitiveShapesManager.getShapes({ location: location, minDistance: 0.1, maxDistance: 2 })
                    if (texts.length) {
                        for (const text of texts) {
                            if (!random) text.color = { red: r, green: g, blue: b, alpha: a }
                            else text.color = { red: Math.random(), green: Math.random(), blue: Math.random(), alpha: a }
                            if (bgr !== undefined && bgb !== undefined && bgg !== undefined) {
                                if (!bgrandom) text.backgroundColorOverride = { red: r, green: g, blue: b, alpha: a }
                                else text.backgroundColorOverride = { red: Math.random(), green: Math.random(), blue: Math.random(), alpha: a }
                            }
                        }
                        return {
                            status: CustomCommandStatus.Success,
                            message: `フロートテキスト の色を変更しました`
                        }
                    }
                    else return {
                        status: CustomCommandStatus.Failure,
                        message: `対象のテキストは見つかりませんでした`
                    }
                }
                else return {
                    status: CustomCommandStatus.Failure,
                    message: `RGBAは 0 ~ 1 の大きさです`
                }
            }
        })
        command.registerCommand({
            name: "xs:textprimitive-attach",
            description: "フロートテキストを追従させます",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [
                { name: "location", type: CustomCommandParamType.Location },
            ...
            if (origin.sourceType === CustomCommandSource.Block || origin.sourceType === CustomCommandSource.Entity) {
                ...
                if (texts.length) {
                    for (const text of texts) {
                        text.attachedTo = targets[0]
                    }
                    return {
                        status: CustomCommandStatus.Success,
                        message: `フロートテキスト を ${targets[0].name} に追従させました`
                    }
                }
                else return {
                    status: CustomCommandStatus.Failure,
        ...
        command.registerCommand({
            name: "xs:textprimitive-scale",
            description: "フロートテキストのサイズを変更します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [
                { name: "location", type: CustomCommandParamType.Location },
                { name: "scale", type: CustomCommandParamType.Float }
            ...
            if (origin.sourceType === CustomCommandSource.Block || origin.sourceType === CustomCommandSource.Entity) {
                if (scale === undefined || (-1000 <= scale && scale <= 1000)) {
                    ...
                    if (texts.length) {
                        ...
                        return {
                            status: CustomCommandStatus.Success,
                            message: `フロートテキスト のスケールを ${scale} にしました`
                        }
                    }
                    else return {
                        status: CustomCommandStatus.Failure,
        ...
        command.registerCommand({
            name: "xs:textprimitive-text",
            description: "フロートテキストの内容を書き換えます",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [{ name: "location", type: CustomCommandParamType.Location }, { name: "text|rawtext", type: CustomCommandParamType.String }],
            optionalParameters: []
        }, (origin, location, texts) => {
            const or = origin.sourceBlock ?? origin.sourceEntity
            const dim = or.dimension
            if (origin.sourceType === CustomCommandSource.Block || origin.sourceType === CustomCommandSource.Entity) {
                const texts = world.primitiveShapesManager.getShapes({ location: location, minDistance: 0.1, maxDistance: 2 })
                if (texts.length) {
                    ...
                    try {
                        for (const text of texts) {
                            text.setText(texts.startsWith("{") ? JSON.parse(texts) : texts)
                        }
                    } catch (e) {
                        er = true;
                    }
        ...
        command.registerCommand({
            name: "xs:textprimitive-location",
            description: "フロートテキストの位置を変更します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [{ name: "location", type: CustomCommandParamType.Location }, { name: "location", type: CustomCommandParamType.Location }],
            optionalParameters: [{ name: "rotation", type: CustomCommandParamType.Location }]
        }, (origin, location, loc, rotation) => {
            const or = origin.sourceBlock ?? origin.sourceEntity
            const dim = or.dimension
            if (origin.sourceType === CustomCommandSource.Block || origin.sourceType === CustomCommandSource.Entity) {
                const texts = world.primitiveShapesManager.getShapes({ location: location, minDistance: 0.1, maxDistance: 2 })
                if (texts.length) {
                    for (const text of texts) {
                        text.setLocation({ x: loc.x, y: loc.y + 0.5, z: loc.z })
                        text.rotation = rotation ?? { x: 0, y: 0, z: 0 }
                    }
                    return {
                        status: CustomCommandStatus.Success,
                        message: `フロートテキスト の位置を変更しました`
                    }
                }
                else return {
                    status: CustomCommandStatus.Failure,
        ...
        command.registerCommand({
            name: "xs:textprimitive-remove",
            description: "フロートテキストを削除します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [
                { name: "location", type: CustomCommandParamType.Location },
            ],
            optionalParameters: [{ name: "attach_target", type: CustomCommandParamType.PlayerSelector }]
        }, (origin, location, targets) => {
            const or = origin.sourceBlock ?? origin.sourceEntity
            const dim = or.dimension
            if (origin.sourceType === CustomCommandSource.Block || origin.sourceType === CustomCommandSource.Entity) {
                const texts = world.primitiveShapesManager.getShapes({ location: targets === undefined ? location : undefined, attachedTo: targets[0], minDistance: 0.1, maxDistance: 2 })
                if (texts.length) {
                    for (const text of texts) {
                        world.primitiveShapesManager.removeText(text)
                    }
                    return {
                        status: CustomCommandStatus.Success,
                        message: `フロートテキスト を削除しました`
                    }
                }
                else return {
                    status: CustomCommandStatus.Failure,
    ...
    if (pack["command:ban"] === true) {
        command.registerCommand({
            name: "xs:queue-ban",
            description: "キューに登録し、プレイヤーのアクセスを禁止します",
            permissionLevel: CommandPermissionLevel.GameDirectors,
            mandatoryParameters: [{ name: "xs:queueBanOptions", type: CustomCommandParamType.Enum }],
            optionalParameters: [{ name: "name", type: CustomCommandParamType.String }]
        }, (origin, que, target) => {
            const sender = origin.sourceEntity
            if (sender instanceof Player || origin.sourceType === CustomCommandSource.Server) {
                if (que === "add" || que === "remove" || que === "list") {
                    if (que === "list") {
                        ...
                        if (lister.length) {
                            ...
                            return {
                                status: CustomCommandStatus.Success,
                                message: `アクセス禁止キュー: (${list.length}件)\n${list.join(" ")}`
                            }
                        }
                        else return {
                            status: CustomCommandStatus.Success,
                            message: `アクセス禁止キュー: (0件)`
                        }
                    }
                    if (que === "add") {
                        const q = queue.create("ban")
                        if (!q.has(target)) {
                            if (sender) {
                                if (sender.name === target) return {
                                    status: CustomCommandStatus.Failure,
                                    message: `自分自身をキューに追加することはできません`
                                }
                                else {
                                    q.add(target)
                                    if (world.getAllPlayers().find(p => p.name === target)) {
                                        const p = world.getAllPlayers().find(p => p.name === target)
                                        system.run(() => { p.runCommand("kick @s `このサーバーへのアクセスが禁止されています`") })
                                        q.remove(target)
                                    }
                                    return {
                                        status: CustomCommandStatus.Success,
                                        message: `${target} をアクセス禁止キューに追加しました`
                                    }
                                }
                            }
                            else {
                            ...
                            status: CustomCommandStatus.Failure,
                            message: `既にそのプレイヤーはキューに追加されています`
                        }
                    }
                    if (que === "remove") {
                        const q = queue.create("ban")
                        if (q.has(target)) {
                            ...
                            return {
                                status: CustomCommandStatus.Success,
                                message: `${target} をアクセス禁止キューから解除しました`
                            }
                        }
                        else return {
                            status: CustomCommandStatus.Failure,
                            message: `そのプレイヤーはキューに追加されていません`
                        }
                    }
                }
                else return {
